<%@ page language="C#" autoeventwireup="true" inherits="MIS_OA_Mail_SelGroup, App_Web_rcdz5rlo" clientidmode="Static" masterpagefile="~/Common/Common.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
#keyText {height: 25px;margin-left: 10px;margin-right: 5px;width: 200px;}
...
<asp:Content runat="server" ContentPlaceHolderID="Content">
                ...
                <ContentTemplate>
                    <div style="float: left; margin-left: 10px; width: 580px; top: 2px;">
                        <div style="margin-bottom: 5px;">
                            ...
                            <asp:TextBox runat="server" CssClass="form-control" ID="keyText" style="display:inline;" onkeydown="return GetEnterCode('click','keyBtn');" placeholder="可输入用户名,会员组名,工号" />
                            ...
                            <asp:Button runat="server" ID="showAll_Btn" Text="全部用户" CssClass="btn btn-primary" Visible="false" OnClick="showAll_Btn_Click" Height="34" />
                            <input type="button" id="AllInfo_sure_btn" value="确定" onclick="AllInfo_sureF();" class="btn btn-primary" />
                          <%--  <input type="button" value="关闭" onclick="$(parent.document).find('#userdiv').hide();" class="btn btn-primary" style="height:34px;float:right;margin-right:5px;"/>--%>
                        </div>
                        <ZL:ExGridView runat="server" ID="EGV" AutoGenerateColumns="false" AllowPaging="true" PageSize="10" EnableTheming="False" CssClass="table table-bordered table-striped" EmptyDataText="当前没有信息!!"
                            ...
                            <Columns>
                                ...
                                <asp:BoundField HeaderText="真实姓名" DataField="HoneyName" />
                            </Columns>
        ...
        <script>
            ...
            function AllInfo_sureF() {
                ...
                if (controlid != "") {//组件调用
                    if ($chkArr[0]) { parent.ZL_UserList.GetSelector(controlid).UserCallBack(CreateJson($chkArr)); }
                    else {alert("请选择用户!") }
                } else {
