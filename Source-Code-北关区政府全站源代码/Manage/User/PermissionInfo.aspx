<%@ page language="C#" autoeventwireup="true" inherits="manage_Config_PermissionInfo, App_Web_jthjv4yd" enableviewstatemac="false" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
    ...
    <ZL:ExGridView runat="server" ID="EGV" AutoGenerateColumns="false" AllowPaging="true" PageSize="20"   EmptyDataText="无数据"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="相关操作">
                <ItemTemplate>
                    <a href="Permissionadd.aspx?menu=edit&id=<%#Eval("ID") %>"><i class="fa fa-pencil"></i> </a>
                    <asp:LinkButton runat="server" CommandArgument='<%#Eval("ID") %>' CommandName="del2" OnClientClick="return confirm('确定要删除吗');"><i class="fa fa-trash" title="删除"></i> </asp:LinkButton>
                    <a href="CompetenceAdd.aspx?id=<%#Eval("ID") %>" >权限设置</a>
                    <a href="SelUsersToRole.aspx?id=<%#Eval("ID") %>" >用户管理</a>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </ZL:ExGridView>
