<%@ page language="C#" autoeventwireup="true" inherits="Manage_User_DepositManage, App_Web_x2mr5qil" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<asp:Content runat="server" ContentPlaceHolderID="head">
    <title>提现管理</title>
</asp:Content>
<asp:Content runat="server" ContentPlaceHolderID="Content">
    <ol class="breadcrumb navbar-fixed-top">
        <li><a href='../Main.aspx'>工作台</a></li><li><a href='UserManage.aspx'>用户管理</a></li><li><a href='<%=Request.RawUrl %>'>用户提现</a></li>
        ...
        <ZL:ExRepeater ID="Deposit_RPT" runat="server" OnItemCommand="Deposit_RPT_ItemCommand" PageSize="10" PagePre="<tr id='page_tr'><td><input type='checkbox' id='chkAll'/></td><td colspan='10' id='page_td'>" PageEnd="</td></tr>">
            <ItemTemplate>
                <tr>
                    ...
                    <td><%#Eval("Y_ID") %></td>
                    <td><a href="javascript:;" onclick="ShowUserDiag(<%#Eval("UserID") %>)"><%#Eval("YName") %></a><input type="hidden" name="uname_hid" value="<%#Eval("YName") %>" /></td>
                    <td><%#Eval("money","{0:f2}") %><input type="hidden" name="price_hid" value="<%#Eval("money") %>" /></td>
                    <td><%#"开户人："+Eval("PeopleName")+",银行："+Eval("Bank")+",卡号："+Eval("Account") %></td>
                    <td><%#GetStatus() %></td>
                    <td><%#Eval("Remark") %></td>
                    <td><%#Eval("Result") %></td>
                    <td>
                        <asp:LinkButton runat="server" CommandName="Check" Text="确认" style="display:none;" CommandArgument='<%#Eval("Y_ID") %>' OnClientClick="return confirm('要确认该笔提现吗?');" CssClass="option_style"><i class="fa fa-money" title="确认提现"></i></asp:LinkButton>
                        <%#GetOP() %>
                    </td>
                </tr>
            </ItemTemplate>
...
<%--            <tr>
    ...
    <asp:Button ID="CheckDepos_B" CssClass="btn btn-primary" runat="server" OnClick="CheckDepos_B_Click" OnClientClick="return confirm('要确认选定提现吗?');" Text="批量确认" />
    <div class="alert alert-info margin_t5">拒绝提现后,请手动为用户增加金额</div>
