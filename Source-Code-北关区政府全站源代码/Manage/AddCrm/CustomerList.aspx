<%@ page language="C#" autoeventwireup="true" inherits="manage_AddCRM_CustomerList, App_Web_ldl5befq" enableviewstatemac="false" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<asp:Content runat="server" ContentPlaceHolderID="Content">
    <asp:Label runat="server" ID="liTitle" Visible="false"></asp:Label>
        ...
        <tbody id="Tbody1"> 
            <tr class="tdbg">
                <td style="height: 24px; width: 5%; font-weight: bold">
                    ID
                </td>
                <td style="height: 24px; width: 15%; font-weight: bold">
                    <%=lang.LF("客户名称")%>
                </td>
                <td  style="height: 24px; width: 8%; font-weight: bold">
                    <%=lang.LF("客户类型")%>
                </td>
                <td style="height: 24px; width: 15%; font-weight: bold">
                    <%=lang.LF("客户编号")%>
                ...
                <td style="height: 24px; width: 10%; font-weight: bold">
                    时间
                </td>
                <td style="height: 24px; width: 10%; font-weight: bold">
                    <%=lang.LF("操作")%>
            ...
            <ZL:ExRepeater ID="ExReater" runat="server" PageSize="10" PagePre="<tr><td><input type='checkbox' id='chkAll'/></td><td colspan='8'><div class='text-center'>" PageEnd="</div></td></tr>">
                <ItemTemplate>
                    <tr class="tdbg" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'" ondblclick="getinfo(<%#Eval("Flow")%>);">
                        <td style="height: 24px;"><%#Eval("Flow")%></td>
                        <td style="height: 24px; cursor:pointer"><a href="ViewCustomer.aspx?FieldName=Person_Add&id=<%#Eval("Flow") %>"><%#Eval("P_name")%></a></td>
                        <td style="height: 24px;"><%#Eval("Client_Type","{0}")=="1"?"企业":"个人"%></td>
                        <td style="height: 24px;"><%#Eval("Code")%></td>
                        <td style="height: 24px;"><%#Eval("Client_Group")%></td>
                        <td style="height: 24px;"><%#Eval("Client_Source")%></td>
                        <td style="height: 24px;"><a href="<%=CustomerPageAction.customPath2 %>User/UserInfo.aspx?id=<%#Eval("FPManID")%>"><%#Eval("UserName")%></td>
                        <td style="height: 24px;"><%#Eval("Add_Date")%></td>
                        <td style="height: 24px;">
                        <a href="ViewCustomer.aspx?FieldName=Person_Add&id=<%#Eval("Flow") %>" class="option_style"><i class="glyphicon glyphicon-eye-open"></i>预览</a> <a href="CustomerManage.aspx?FieldName=Person_Add&menu=edit&id=<%#Eval("Flow") %>" class="option_style"><i class="glyphicon glyphicon-edit"></i>修改</a> <a href="?menu=delete&code=<%#Eval("Code") %>" onclick="return confirm('你确定要将所有选择删除吗？');"  class="option_style"><i class="glyphicon glyphicon-trash"></i>删除</a></td>
                    </tr>
                </ItemTemplate>
