<%@ page language="C#" autoeventwireup="true" inherits="Manage_I_Content_LabelManage, App_Web_pwtmdret" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
        ...
        <ItemTemplate>
            <tr ondblclick="ckl('<%# Eval("LabelName") %>')">
                ...
                <td class="text-center" id="select">
                    <a class="option_style" href="LabelSql.aspx?LabelName=<%#HttpUtility.UrlEncode(Eval("LabelName","")) %>"><i class="fa fa-pencil" title="修改"></i></a>
                    <asp:LinkButton CssClass="option_style" runat="server" CommandName="Copy" CommandArgument='<%# Eval("LabelName") %>' OnClientClick="return confirm('确实要复制此标签吗？');"><i class="fa fa-copy" title="复制"></i></asp:LinkButton>
                    ...
                    <asp:LinkButton CssClass="option_style" runat="server" CommandName="Del" CommandArgument='<%# Eval("LabelName") %>'  OnClientClick="return confirm('确实要删除此标签吗？');"><i class="fa fa-trash-o" title="删除"></i></asp:LinkButton>
                    <a class="option_style" href="../APP/AddJsonP.aspx?LabelID=<%#Eval("LabelID") %>"  title="JsonP跨域调用"><i class="fa fa-list"></i>调用</a>
