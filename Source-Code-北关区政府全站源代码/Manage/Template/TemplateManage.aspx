<%@ page language="C#" autoeventwireup="true" inherits="Manage_I_Content_TemplateManage, App_Web_pwtmdret" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<asp:Content runat="server" ContentPlaceHolderID="Content">
        ...
        <asp:Repeater ID="repFile" runat="server" OnItemCommand="repFileReName_ItemCommand">
            <ItemTemplate>
                <tr class="tdbg" onmouseover="this.className='tdbgmouseover'"  ondblclick="dd('<%# Eval("Name").ToString()%>')"  id="chk_<%# Eval("Name").ToString()%>" onmouseout="this.className='tdbg'">
                    ...
                    <li><asp:LinkButton ID="LinkButton3" CssClass="option_style" runat="server" CommandArgument='<%# Eval("Name").ToString()%>' CommandName='<%# System.Convert.ToInt32(Eval("type")) == 1 ? "":"DownFiles" %>' Visible='<%#Eval("content_type").ToString() == "config" ? false:true %>'><i class="fa  fa-download"></i>下载</asp:LinkButton></li>
                    <li><asp:LinkButton ID="LinkButton1" CssClass="option_style" runat="server" CommandArgument='<%# Eval("Name").ToString()%>' CommandName='<%# System.Convert.ToInt32(Eval("type")) == 1 ? "DelDir":"DelFiles" %>' OnClientClick="return confirm('你确认要删除该文件夹或文件吗？')"><i class="fa fa-trash-o"></i>删除</asp:LinkButton></li>
                    <li><asp:LinkButton ID="LinkButton2" CssClass="option_style" runat="server" CommandArgument='<%# Eval("Name").ToString()%>' CommandName='<%# System.Convert.ToInt32(Eval("type")) == 1 ? "CopyDir":"CopyFiles" %>' Enabled='<%# System.Convert.ToInt32(Eval("type")) == 1 ? false:true %>'><i class="fa fa-copy"></i>复制</asp:LinkButton></li>
                    </ul>
                    </div>                      
                    </td>
                    <td align="left">
   ...
   <a runat="server" id="ViewEdit_A" class="btn btn-primary" target="_blank"><i class="fa fa-edit" style="margin-right:5px;"></i>可视化编辑</a>
