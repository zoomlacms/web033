<%@ page language="C#" autoeventwireup="true" inherits="manage_Page_PageManage, App_Web_wob3d5vk" enableviewstatemac="false" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<asp:Content runat="server" ContentPlaceHolderID="Content">
    <ol id="BreadNav" class="breadcrumb navbar-fixed-top">
        ...
        <li>黄页列表</li>
        <li><div class="input-group nav_searchDiv pull-right">
        ...
        <div id="help" class="pull-right"><a onclick="help_show('86')" title="帮助"><span class="glyphicon glyphicon-question-sign"></span></a></div>
    </ol>
        ...
        <ZL:ExRepeater ID="RPT" runat="server" PageSize="10"  PagePre="<tr id='page_tr'><td colspan='11''><label class='allchk_l'><input type='checkbox' id='chkAll'/>全选</label><div class='text-center'>" PageEnd="</div></td></tr>">
            <ItemTemplate>
                <tr id="<%#Eval("ID") %>" ondblclick="getinfo(this.id)">  
                    <td align="left"><input name="idchk" type="checkbox" value="<%#Eval("ID")%>" /></td>
                    <td><%#Eval("ID")%></td>
                    ...
                    <td>
                        ...
                        <a href="AuditUser.aspx?menu=modify&id=<%#Eval("ID") %>"><i class="glyphicon glyphicon-pencil" title="修改"></i></a> 
                        <a href="?menu=del&id=<%#Eval("ID") %>" onclick="return confirm('不可恢复性删除数据,你确定将该数据删除吗');"><i class="glyphicon glyphicon-trash" title="删除"></i></a>
                        <a href="PageTemplate.aspx?StyleID=<%#Eval("NodeStyle") %>" title="栏目管理"><i class="fa fa-list-ul"></i></a>
                        <%#GetRecommendation()%>
                        <%#GetStatus()%>
                    </td>
                </tr>
