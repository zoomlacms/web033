<%@ page language="C#" autoeventwireup="true" inherits="ZoomLa.WebSite.Manage.Page.PageContent, App_Web_wob3d5vk" enableviewstatemac="false" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
            ...
            <ZL:ExRepeater ID="RPT" runat="server" PageSize="10" PagePre="<tr><td colspan='9'><label class='allchk_l'><input type='checkbox' id='chkAll'/>全选</label><div class='text-center'>" PageEnd="</div></td></tr>"
                ...
                <ItemTemplate>
                    <tr>
                        ...
                        <td>
                            <a href="<%#GetUrl(Eval("GeneralID").ToString()) %>" class="option_style" target="_blank" title="预览"><span class="fa fa-eye"></span></a>
                            <asp:LinkButton ID="LinkButton2" runat="server" ToolTip="修改" CommandName="Edit" CssClass="option_style" CommandArgument='<%# Eval("GeneralID") %>'><i class="glyphicon glyphicon-pencil" title="修改"></i></asp:LinkButton>
                             <asp:LinkButton ID="LinkButton3" runat="server" CssClass="option_style" CommandName="Del" CommandArgument='<%# Eval("GeneralID") %>' OnClientClick="return confirm('你确定将该数据删除到回收站吗？')"><i class="glyphicon glyphicon-trash" title="删除"></i> 删除</asp:LinkButton>
                        </td>
                    </tr>
                </ItemTemplate>
