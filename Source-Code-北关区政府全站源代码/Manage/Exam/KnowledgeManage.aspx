<%@ page language="C#" autoeventwireup="true" masterpagefile="~/Manage/I/Default.master" inherits="Manage_Exam_KnowledgeManage, App_Web_yercog2q" enableEventValidation="false" viewStateEncryptionMode="Never" %>
    ...
    <ZL:Repeater ID="RPT" runat="server" OnItemCommand="RPT_ItemCommand">
        <ItemTemplate>
            <tr data-layer="1" data-id='<%#Eval("k_id") %>'>
                ...
                <td><%#Eval("CDate") %></td>
                <td>
                    <a href='AddKnowledge.aspx?id=<%#Eval("k_id") %>&nid=<%=NodeID %>' title='修改'><span class='glyphicon glyphicon-pencil option_style'></span></a>
                    <asp:LinkButton runat="server" CommandName="del2" CommandArgument='<%#Eval("k_id") %>'><i class='glyphicon glyphicon-trash option_style'></i></asp:LinkButton>
...
<asp:Content runat="server" ContentPlaceHolderID="ScriptContent">
    ...
    <script>
        ...
        function GetOption(id) {
            ...
            return "<a href='AddKnowledge.aspx?id="+id+"' title='修改'><span class='glyphicon glyphicon-pencil'></span></a> "
               + "<a href='javascript:;' onclick='DelCofim("+id+")'><span class='glyphicon glyphicon-trash'></span></a> "
        ...
        function GetIcon(count, id, typename) {
            var classname = count > 0 ? "glyphicon glyphicon-folder-close" : "glyphicon glyphicon-file";
