<%@ page language="C#" autoeventwireup="true" inherits="manage_Question_Question_Class_Manage, App_Web_yercog2q" enableviewstatemac="false" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
        ...
        <ZL:Repeater ID="RPT" runat="server">
            <ItemTemplate>
              <tr data-layer="1" data-id='<%#Eval("C_Id") %>' onclick='LoadChilds(this,<%#Eval("C_Id") %>)'>
                  ...
                  <td>
                      <a href='AddQuestion_Class.aspx?Action=Modify&ID=<%# Eval("C_Id")%>&Pid=<%# Eval("C_Classid")%>' title="修改"><span class="glyphicon glyphicon-pencil"></span></a>
                      <a href="javascript:;" onclick='DelCofim(<%# Eval("C_Id")%>)' title="删除"><span class="glyphicon glyphicon-trash"></span></a>
                      <a href='AddQuestion_Class.aspx?Action=Add&ID=<%# Eval("C_Id")%>'>添加子科目</a>
                      <a href="KnowledgeManage.aspx?nid=<%#Eval("C_Id") %>">知识点列表</a>
                      <a href="AddKnowledge.aspx?nid=<%#Eval("C_Id") %>">添加知识点</a>
                      <a href='QuestionManage.aspx?NodeID=<%#Eval("C_Id") %>'>试题内容</a>
                  </td>
              </tr>
            </ItemTemplate>
        </ZL:Repeater>
...
<script src="/JS/Controls/ZL_Array.js"></script>
    ...
    <script>
        var Temp = "<tr data-layer='@layer' data-pid='@C_Classid' data-id='@C_id' onclick='LoadChilds(this,@C_id)'>"
                ...
                + " <td>@ParentName</td><td><a href='AddQuestion_Class.aspx?Action=Modify&ID=@C_id&Pid=@C_Classid' title='修改'><span class='glyphicon glyphicon-pencil'></span></a>"
                + " <a href='javascript:;' onclick='DelCofim(@C_id)' title='删除'><span class='glyphicon glyphicon-trash'></span></a>"
        ...
        function ReplaceTlp(Temp, layer, data) {
            ...
            for (var i = 0; i < data.length; i++) {
                var curtemp = Temp.replace(/@ParentName/g,data[i].ParentName==""?"无所属科目":data[i].ParentName).replace(/@imgicon/g, data[i].ChildCount > 0 ? "glyphicon glyphicon-folder-close" : "glyphicon glyphicon-file");
