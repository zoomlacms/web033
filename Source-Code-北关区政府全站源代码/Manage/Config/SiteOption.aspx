<%@ page language="C#" autoeventwireup="true" inherits="Manage_I_Config_SiteOption, App_Web_rfihykcf" masterpagefile="~/Manage/I/Default.master" enableviewstatemac="false" validaterequest="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
	...
	<tbody>
		...
		<tr>
			...
			<td>
				...
				<span class="rspan">支持从远程云端下载功能与模板界面，官方服务器地址：http://update.zoomla.cn</span></td>
		</tr>
		<tr>
		...
		<tr>
			<td><strong><%:lang.LF("店铺首页模板") %>：</strong></td>
			<td>
		...
		<tr>
			<td><strong><%:lang.LF("模板方案") %>：</strong></td>
			<td>
...
<%--					<asp:Button ID="Button2" Style="margin-left: 5px;" OnClick="Button2_Click" OnClientClick="document.getElementById('lblKai').innerText='开销中...';" runat="server" Text="一键开销" CssClass="btn btn-primary" UseSubmitBehavior="false" />
		...
		<tr>
			<td><strong><%:lang.LF("风格路径") %>：</strong></td>
			<td>
				<div id="DivCssDir">
		...
		<tr>
			<td><strong><%:lang.LF("默认风格") %>：</strong></td>
			<td>
...
<table class="table table-striped table-bordered table-hover">
	<tbody>
        ...
        <tr>
            <td><strong>HTTPS全站SSL加密：</strong></td>
            <td>
               <input type="checkbox" runat="server" class="switchChk" id="safeDomain_Chk" />
               <span class="rd_red">*是否启用SSL加密连接(如未配置证书,请勿开启)</span>
            </td>
        </tr>
		<tr>
