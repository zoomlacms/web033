<%@ page language="C#" autoeventwireup="true" inherits="manage_IISManage_CreateSite, App_Web_arjfynwg" masterpagefile="~/Manage/I/Default.master" title="创建新站点" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<div id="CSWSDiv" class="cswsDiv"><!--Create Single Web Site-->
	 ...
	 <li><strong>物理路径：</strong><asp:TextBox runat="server" CssClass="form-control text_md" ID="CSPhysicalPath" onblur="checkValue(this,'R2',2)"/><span style="color:red" id="R2">*非自动模式，必须在服务器上已经建立好文件夹，如:D:\WebSite\</span></li>
		 ...
		 <li><strong>动态域名：</strong><asp:TextBox runat="server" ID="CSDomain" CssClass="form-control text_md"  txt="域名"/><span style="color:red">*例:www.zoomla.cn,需要绑定的域名</span></li>
  ...
  <script type="text/javascript">
	  ...
	  function newadd() {
	      var tr = "<li><strong><label style='color:#999;'>批量绑定：</label></strong></td><td><label>端口：</label><input type='text' name='sitePort' value='80' class='form-control text_md'   maxlength='6' txt='端口'/> " +
			 ...
			 "<i class='glyphicon glyphicon-remove'></i></a></li>";
		  $("#batTr").after(tr);
	  }
	  function remove(obj) {
		  $(obj).parent().remove();
