<%@ page language="C#" autoeventwireup="true" inherits="IISMain, App_Web_35vgvlcp" masterpagefile="~/Manage/I/Default.master" title="应用程序池列表" enableEventValidation="false" viewStateEncryptionMode="Never" %>
    ...
    <div id="tab3">
         <%--   <asp:UpdatePanel runat="server" ID="AppPoolPanel">
                <ContentTemplate>--%>
                    <ZL:ExGridView ID="EGV4" runat="server" DataSourceID="AppPoolData" AllowPaging="True" RowStyle-CssClass="tdbg" AutoGenerateColumns="false"
                       ...
                       <%#(Eval("State") as string) == "Started" ? "<i class='glyphicon glyphicon-play'></i>" 
       ...
       <%--                         <EditItemTemplate>
                            ...
                            <asp:TemplateField HeaderText="操作" ItemStyle-CssClass="site_a">
                                <ItemTemplate>
                                  <asp:LinkButton ID="LinkButton2" runat="server" CommandName="Edit2" CommandArgument='<%# Container.DisplayIndex %>' CssClass="option_style"><i class="glyphicon glyphicon-pencil" title="修改"></i></asp:LinkButton>
                                   ...
                                   <a href="javascript:if(confirm('你确定要删除吗!')){ postToCS('del','<%#Eval("AppPoolName") %>')}">停止</a>
                                    ...
                                    <asp:LinkButton runat="server" CommandName="stop" CommandArgument='<%#Eval("AppPoolName") %>' Visible='<%#(Eval("State","").Equals("Started"))%>' CssClass="option_style"><i class="glyphicon glyphicon-pause" title="停止"></i>停止</asp:LinkButton>
                                    <asp:LinkButton  runat="server" CommandName="start" CommandArgument='<%#Eval("AppPoolName") %>' Visible='<%#!(Eval("State","").Equals("Started"))%>' CssClass="option_style"><i class="glyphicon glyphicon-play" title="启动"></i>启动</asp:LinkButton>
                                </ItemTemplate>
                                <EditItemTemplate>
