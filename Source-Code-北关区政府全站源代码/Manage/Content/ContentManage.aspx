<%@ page language="C#" autoeventwireup="true" inherits="Manage_I_Content_ContentManage, App_Web_zz5lfliw" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
		...
		<tr ondblclick="location='ShowContent.aspx?GID=<%#Eval("GeneralID") %>';">
			...
			<td>
				...
				<asp:LinkButton runat="server" CssClass="option_style" CommandName="Edit" CommandArgument='<%# Eval("GeneralID") %>' 
                    Enabled='<%# getenabled(Eval("Status").ToString()) %>' CausesValidation="false"><i class="fa fa-pencil" title="修改"></i></asp:LinkButton>  
				<asp:LinkButton runat="server" ID="lbDelete" CssClass="option_style" Enabled='<%#getenabled(Eval("GeneralID").ToString()) %>' CommandName="Del" 
                    CommandArgument='<%# Eval("GeneralID") %>' OnClientClick="return confirm('你确定将该数据删除到回收站吗?');"><i class="fa fa-trash" title="删除"></i></asp:LinkButton>   
				<a class="option_style" href="/Item/<%#Eval("GeneralID") %>.aspx" target="_blank"><i class='fa fa-globe'> </i>浏览</a> 
