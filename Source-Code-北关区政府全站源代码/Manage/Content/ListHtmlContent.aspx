<%@ page language="C#" masterpagefile="~/Manage/I/Default.master" autoeventwireup="true" inherits="Manage_I_Content_ListHtmlContent, App_Web_zz5lfliw" clientidmode="Static" validaterequest="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="Content" runat="Server">
    ...
    <div class="panel panel-default" style="padding:0px;">
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="生成操作">
                <ItemTemplate>
                    ...
                    <asp:LinkButton ID="LinkButton1" runat="server" CommandName="MakeHtml" CommandArgument='<%# Eval("GeneralID") %>' CausesValidation="false" CssClass="option_style"><i class="fa fa-spinner" title="生成"></i>生成文件</asp:LinkButton>
                    <a href="<%#(geturl(Eval("GeneralID","{0}"))=="")?"javascript:void(0);":geturl(Eval("GeneralID","{0}"))%>" class="option_style"><i class="fa fa-eye" title="预览" ></i>查看文件</a>
				<a href="ListHtmlContent.aspx?GeneralID=<%# Eval("GeneralID") %>" class="option_style"><i class="fa fa-trash-o" title="删除"></i></a>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Center" />
            </asp:TemplateField>
        </Columns>
        ...
        <div class="panel panel-footer" style="padding:3px; margin:0px;">
            <asp:Button ID="btnAudit" CssClass="btn btn-primary" CausesValidation="false" runat="server" Text="批量生成文件" OnClientClick="if(!IsSelectedId()){alert('请选择生成项');return false;}else{return confirm('你确定要生成选中内容吗？')}" OnClick="btnAudit_Click" />&nbsp;
            <asp:Button ID="btnUnAudit" CausesValidation="false" runat="server" Text="批量删除文件" CssClass="btn btn-primary" OnClientClick="if(!IsSelectedId()){alert('请选择删除项');return false;}else{return confirm('你确定要取消审核选中内容吗？')}" OnClick="btnUnAudit_Click" />&nbsp;
