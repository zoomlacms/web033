<%@ page language="C#" autoeventwireup="true" inherits="ZoomLa.WebSite.Manage.Content.CollectionManage, App_Web_zz5lfliw" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
    ...
    <ZL:ExGridView ID="Egv" runat="server" AutoGenerateColumns="False" DataKeyNames="CItem_ID" OnPageIndexChanging="Egv_PageIndexChanging" OnRowCommand="Egv_RowCommand"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="操作">
                <ItemTemplate>
                    <a href='CollectionStep1.aspx?Action=Modify&ItemID=<%#DataBinder.Eval (Container.DataItem,"CItem_ID")%>' class="option_style"><i class="fa  fa-pencil" title="修改"></i>修改项目</a> 
                    <a href='CollectionStep2.aspx?Action=Modify&ItemID=<%#DataBinder.Eval (Container.DataItem,"CItem_ID")%>' class="option_style"><i class="fa  fa-pencil" title="修改"></i>修改列表</a> 
                    <a href='CollectionStep3.aspx?Action=Modify&ItemID=<%#DataBinder.Eval (Container.DataItem,"CItem_ID")%>' class="option_style"><i class="fa  fa-pencil" title="修改"></i>修改字段</a> 
                    <asp:LinkButton runat="server" CommandName="Del2" CommandArgument='<%# Eval("CItem_ID") %>' OnClientClick="return confirm('你确定将该数据删除吗？')" class="option_style"><i class="fa fa-trash-o" title="删除"></i></asp:LinkButton>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Center" />
            </asp:TemplateField>
        </Columns>
