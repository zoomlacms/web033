<%@ page language="C#" autoeventwireup="true" inherits="ZoomLa.WebSite.Manage.Content.AuditingState, App_Web_zz5lfliw" enableviewstatemac="false" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<asp:Content runat="server" ContentPlaceHolderID="Content">
	...
	<Columns>
		...
		<asp:TemplateField HeaderText="编辑" ShowHeader="False"  ItemStyle-HorizontalAlign="Center">
			<ItemTemplate >
				<asp:LinkButton ID="btnEdit" runat="server" CausesValidation="True" CommandName="Edit" Enabled='<%# IsEnabled(Eval("StateType").ToString())%>' CssClass="option_style"><i class="fa fa-pencil" title="编辑"></i></asp:LinkButton>
			</ItemTemplate>
			<EditItemTemplate>
		...
		<asp:TemplateField HeaderText="删除" ShowHeader="False"  ItemStyle-HorizontalAlign="Center">
			<ItemTemplate>
				<asp:LinkButton ID="btnDel" runat="server" CommandName="Delete" OnClientClick="return confirm('你确定将该数据删除吗？');"  CommandArgument='<%# Eval("StateCode")%>' Enabled='<%# IsEnabled(Eval("StateType").ToString())%>' CssClass="option_style"><i class="fa fa-trash-o" title="删除"></i></asp:LinkButton>
			</ItemTemplate>
		</asp:TemplateField>
	</Columns>
