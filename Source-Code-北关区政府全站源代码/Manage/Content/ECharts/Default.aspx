<%@ page language="C#" autoeventwireup="true" inherits="Manage_Content_ECharts_Default, App_Web_0qmbteha" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
    ...
    <ZL:ExGridView ID="EGV" runat="server" AutoGenerateColumns="False" PageSize="10" OnRowCommand="EGV_RowCommand"
        ...
        <Columns> 
            ...
            <asp:TemplateField HeaderText="操作">
                <ItemTemplate>
                   <a href="/Plugins/ECharts/ShowM.aspx?ID=<%#Eval("ID") %>" target="_blank" title="预览" class="option_style"><span class="fa fa-eye"></span></a>
                    <a href="AddChart2.aspx?<%#"id="+Eval("ID")+"&type="+Eval("SType")+"&tag="+Eval("Tag") %>" class="option_style"><span class="glyphicon glyphicon-pencil" title="编辑"></span> </a>
                    <asp:LinkButton runat="server" CommandArgument='<%#Eval("ID") %>' class="option_style" CommandName="del2" OnClientClick="return confirm('确定要删除吗?');"><span class="glyphicon glyphicon-trash" title="删除"></span> </asp:LinkButton>
