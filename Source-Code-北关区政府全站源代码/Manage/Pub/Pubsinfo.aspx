<%@ page language="C#" masterpagefile="~/Manage/I/Default.master" autoeventwireup="true" inherits="Manage_I_Pub_Pubsinfo, App_Web_nsadxgpt" enableEventValidation="false" viewStateEncryptionMode="Never" %>
        ...
        <ZL:ExGridView ID="Egv" runat="server" AutoGenerateColumns="False" DataKeyNames="ID" PageSize="10" OnRowDataBound="Egv_RowDataBound" OnPageIndexChanging="Egv_PageIndexChanging" IsHoldState="false" OnRowCommand="Lnk_Click" AllowPaging="True" AllowSorting="True" CssClass="table table-striped table-bordered table-hover" EnableTheming="False" EnableModelValidation="True" EmptyDataText="暂无互动信息！">
            <Columns>
                ...
                <asp:TemplateField HeaderText="操作">
                    ...
                    <ItemTemplate>
                        <a href="ViewSmallPub.aspx?Pubid=<%:PubID %>&ID=<%# Eval("ID", "{0}")%><%= this.HiddenNode.Value %>&type=0 ">相关信息与回复管理</a>
                        |
                        <a href="AddPub.aspx?Pubid=<%:PubID%>&Parentid=<%#Eval("ID") %><%= this.HiddenNode.Value %>&type=<%=Request["type"] %>">回复</a>&nbsp;
                        |
