<%@ page language="C#" autoeventwireup="true" inherits="Plat_Doc_CompDoc, App_Web_vs51lm20" masterpagefile="~/Plat/Main.master" validaterequest="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
	...
	<ItemTemplate>
	  <tr id="file_tr_<%#Eval("ID") %>" onmouseover="$(this).find('.hoverDiv').show();" onmouseout="$(this).find('.hoverDiv').hide();">
		<td><div style="position:relative;bottom:-10px;display:inline-block;"> <%#GroupPic.GetExtNameMini(GetExt()) %> </div>
		  ...
		  <%#GetOP() %> <a><span class='glyphicon glyphicon-refresh' title='重命名' onclick="ShowReName(<%#Eval("ID") %>);"></span></a> <a><span class='glyphicon glyphicon-trash' title='删除' onclick="PostDel(<%#Eval("ID")+",'"+Eval("FileName")+"'" %>);"></span></a> </span></td>
