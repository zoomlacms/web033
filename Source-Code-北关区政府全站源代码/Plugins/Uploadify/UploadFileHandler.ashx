        ...
        try
        {
            switch (action)
            {
                ...
                case "Cloud_Doc":
                    ...
                    uploadPath = context.Server.UrlDecode(SiteConfig.SiteOption.UploadDir+"YunPan/" + mu.UserName + mu.UserID +value+ "/");
                    break;
            ...
            else if (action.Equals("Cloud_Doc"))
            {
                ...
                cloudMod.FileType = 1;
                result = SafeSC.SaveFile(uploadPath, file, cloudMod.SFileName);
                ...
                cloudMod.FileSize = new FileInfo(context.Server.MapPath(uploadPath)+cloudMod.SFileName).Length.ToString();
            ...
            else
            {
                switch ((context.Request["upmode"] ?? ""))//组图,多图上传
                {
                    case "1"://压缩包上传
                        ...
                        break;
                    default:
                        if (SafeC.IsImageFile(file.FileName) && file.ContentLength > (100 * 1024))//图片超过100K则压缩
                        {
                            result = uploadPath + file.FileName;
                            new ImgHelper().CompressImg(file, 100, result);
                        }
                        else
                        {
